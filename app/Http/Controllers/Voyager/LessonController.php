...
class LessonController extends VoyagerBaseController
{
    ...
    public function create_question(Request $request) {
        ...
        if (!$lesson_id || $lesson_id == 0) return;
    ...
    public function create_answer(Request $request) {
        ...
        $answer->name = "Новый ответ на вопрос " . $question_id;
    ...
    public function get_test_items(Request $request) {
        ...
        return $lesson->renderQuestions(null);
    }

    public function reorder_test_items(Request $request) {
        $test = Lesson::find($request->lesson_id);
        $order = $request->order;
        if (!$test) return "error";
        if (!$order) return "error";
        if($test->reorderTest($order)) return "done";
        return "error";        
    }
